...
app.config['MYSQL_USER'] = 'note_keeper'
app.config['MYSQL_PASSWORD'] = ''
app.config['MYSQL_DB'] = 'NoteKeeperDB'


@login_manager.user_loader
def load_user(user_id: int):
    return get_user(int(user_id), mysql)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        success = add_user(email, password, mysql)
        if success:
...
def login():
    ...
    if form.validate_on_submit():
        email = form.email.data
        password = form.password.data
        user = user_exist(email, password, mysql)
        if user:
            login_user(user)
...
def notes():
    ...
    user_notes = read_latest_100_notes(user_id, mysql)
...
def add_note():
    ...
    if form.validate_on_submit():
        user_id = current_user.id
        create_note(user_id, form.note.data, mysql)
...
def edit_note(note_id: int):
    user_id = current_user.id
    cur_note = get_note(user_id, note_id, mysql)
    form = NoteForm(obj=cur_note)
    if form.validate_on_submit():
        update_note(user_id, note_id, form.note.data, mysql)
...
def delete_note(note_id: int):
    user_id = current_user.id
    remove_note(user_id, note_id, mysql)
